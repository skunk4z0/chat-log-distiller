...
- [x] **チャンク単位キャッシュ & レジューム**: `output/.cache_<元名>.md/chunk_0000.json` 形式で逐次保存し、既存キャッシュはAPIをスキップ（`--only` 再実行を日付跨ぎでも継続可能）
...
- [x] **超アグレッシブ・ルーティング（チャンク内）**: 503/429 の一時エラー時は待機せず即時に次点プロバイダへフォールバックし、全候補が一時失敗した時だけバックオフ
- [x] **`--only` 実行の即時終了**: 対象ファイルの処理完了（成功または failed 退避）直後にプロセスを終了し、アーカイブ後の再探索ループを防止
